...
`node scripts/bake-sprites.mjs --out=DIR [--v2] [--fx]` draws a preview into
DIR without touching the shipped sprites — `--v2` with the ART_V2 redesign
(`art-flag.ts`, still off in production), `--fx` with the bake's finishing
pass (rim light, under-shading, bloom; `GRADE` in the script, also still off).
