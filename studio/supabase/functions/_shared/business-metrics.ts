...
//
// Kept byte-identical with lib/business-metrics.ts (same precedent as
// _shared/categories.ts <-> lib/category-sync.test.ts, _shared/schedule.ts
// <-> lib/compute-next-run.ts) so it's both Deno-importable and
// vitest-testable from the same source, verified by
