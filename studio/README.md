...
- Post ทุกช่องทาง (`/post`) — one-click multi-channel queue. Auto-publish now works for **Facebook** (Page feed), **LINE** (broadcast), **Instagram** (image, via the Meta Page token), **TikTok** (photo/video via PULL_FROM_URL — needs a public media URL) and **X/Twitter** (text + media ≤5MB, OAuth 1.0a) once each channel is connected in Settings → Integrations (`tiktok-oauth-start/callback`, `x-oauth-start/callback`). **YouTube** still posts by hand via the direct link in the queue.
- **Email channel (Resend)** — `email-send` / `email-campaign` edge functions plus invoice/receipt emails wired into the payment flow (`_shared/payments.ts`): ใบแจ้งชำระอัตโนมัติเมื่อสร้าง payment และใบเสร็จเมื่อยืนยันเงิน ถ้าลูกค้ามี `customers.email` (column added in migration 0083). จดหมายข่าวส่งได้จาก Settings → Integrations → Email. ต้องตั้ง `RESEND_API_KEY` + `email_from_address`.
- **ทีม AI ขยายเป็น 7 agents** — CEO Agent มอบงานให้ Sales, Marketing, Finance, **Content**, **Ops**, **Research** และ Business Analyst วิเคราะห์คู่ขนาน (เพิ่ม action types อีก 3: `draft_content` รันอัตโนมัติ, `update_customer` / `send_email` รออนุมัติ) — migration 0083
