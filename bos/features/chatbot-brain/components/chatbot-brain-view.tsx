              ...
              filteredDocs.map((doc) => (
                <div key={doc.id} className="flex items-center justify-between rounded-lg px-2 py-1.5 text-xs hover:bg-line/5 group cursor-pointer" onClick={() => setSelectedDoc(doc)}>
                  ...
                  <div className="flex shrink-0 items-center gap-1">
                    <Button variant="ghost" size="icon" className="h-6 w-6 text-secondary/20 opacity-0 group-hover:opacity-100 hover:text-primary transition-opacity" onClick={(e) => { e.stopPropagation(); setSelectedDoc(doc); }} title="ดูรายละเอียด">
                      <Eye className="h-3 w-3" />
                    </Button>
                    <Button variant="ghost" size="icon" className="h-6 w-6 text-secondary/20 opacity-0 group-hover:opacity-100 hover:text-danger transition-opacity" onClick={(e) => { e.stopPropagation(); handleDeleteDoc(doc.id); }} title="ลบ">
