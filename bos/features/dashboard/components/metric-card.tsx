...
export function MetricCard({ label, value, icon: Icon, tone = "purple", subtext, subtextPositive = true, href }: MetricCardProps) {
  ...
  const content = (
    <div className="group rounded-2xl border border-white/5 bg-card p-5 transition-all duration-150 hover:border-white/10 hover:bg-white/[0.03]">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          {subtext ? (
            <p className={cn("mt-1 text-xs font-medium", subtextPositive ? "text-emerald-400" : "text-orange-400")}>{subtext}</p>
          ) : null}
        </div>
